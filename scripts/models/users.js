(function (module) {
  ...
  User.prototype.sitesNearAddress = function(distanceFilter, address, quantity) {
    var geocoder = new google.maps.Geocoder();
    tempQuestList = [];
    geocoder.geocode( { 'address': address}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        ...
        console.log('end of sitesNearAddress2');
        console.log('sorted list: ', sortedByDistance);
        tempQuestList = sortedByDistance;
        console.log(nearMe);
  ...
  //   console.log(distanceFilter, address);
